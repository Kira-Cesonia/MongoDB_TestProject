...
class MongoDB_ReaderTest {
	...
	private static final int DANI_ENTRY_COUNT = 3;
	private static final int HENRY_ENTRY_COUNT = 2;
	...
	static void setUpMongoDB_reader() throws DatabaseNotConnectedException {
		...
		for (int i = 0; i < DANI_ENTRY_COUNT; i++) {
			GameCharacter dani = GameCharacters.dani();
			mongoDB_Writer.writeCharacterToMongoDB(dani);
		}
		for (int i = 0; i < HENRY_ENTRY_COUNT; i++) {
			GameCharacter henry = GameCharacters.henry();
			mongoDB_Writer.writeCharacterToMongoDB(henry);
		}
	}
	...
	void readCharacterFromMongoDB_byIndex_shouldReturnCharacterWithCorrectName() {
		GameCharacter testCharacter
			= mongoDB_Reader.readCharacterFromMongoDB_byIndex(TEST_CHARACTER_INDEX);
	...
	void readCharacterFromMongoDB_byID_shouldReturnCharacterWithCorrectName() {
		GameCharacter testCharacter
			= mongoDB_Reader.readCharacterFromMongoDB_byID(TEST_CHARACTER_ID);
	...
	@Test
	void readGameCharactersByElementShouldReturnCorrectObjects() {
		Element element = GameCharacters.dani().element;
		
		List<GameCharacter> gameCharacterList
			= mongoDB_Reader.readGameCharactersByElement(element);
		
		assertEquals(DANI_ENTRY_COUNT, gameCharacterList.size());
	}
